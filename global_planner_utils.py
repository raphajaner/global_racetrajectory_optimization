...
def smooth_centerline(centerline: np.ndarray) -> np.ndarray:
    ...
    Afterward, we take the results of the second smoothed centerline for the beginning and end of the
...
def write_centerline(centerline: np.ndarray, sp_bool: bool = False):  # -> MarkerArray:
    ...
    with open(cent_path, 'w', newline='') as file:
        ...
        for row in centerline:
            ...
            # cent_marker = Marker()
            cent_marker = Namespace(header=Namespace(frame_id='map'),
                                    type=Namespace(SPHERE=0),
                                    scale=Namespace(x=0.05, y=0.05, z=0.05),
                                    color=Namespace(a=1.0, b=1.0, r=0.0, g=0.0),
                                    pose=Namespace(position=Namespace(x=x_m, y=y_m, z=0.0),
                                                   orientation=Namespace(w=1.0)),
                                    id=id_cnt,
                                    SPHERE=0
                                    )
            ...
            # wpnt = Wpnt()
            wpnt = Namespace(id=0, x_m=0.0, d_right=0.0, d_left=0.0, y_m=0.0)
            wpnt.id = id_cnt
            wpnt.x_m = x_m
